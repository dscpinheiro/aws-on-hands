...
namespace LambdaApi.Tests
{
    public class S3ProxyControllerTests : IDisposable
    {
        ...
        public S3ProxyControllerTests()
        {
            ...
            _s3Client = _configuration.GetAWSOptions().CreateServiceClient<IAmazonS3>();

            // Create a bucket used for the test which will be deleted along with any data in the bucket once the test is complete.
            _bucketName = "lambda-S3ProxyControllerTests-".ToLower() + DateTime.Now.Ticks;
            _s3Client.PutBucketAsync(_bucketName).Wait();
        }
        ...
        public async Task TestSuccessWorkFlow()
        {
            ...
            Startup.Configuration[Startup.AppS3BucketKey] = _bucketName;
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!disposedValue)
            {
                if (disposing)
                {
                    AmazonS3Util.DeleteS3BucketWithObjectsAsync(_s3Client, _bucketName).Wait();
                    _s3Client.Dispose();
                }
